...
module.exports = async (client, message) => {
  if(!message.guild) 
    return message.channel.send("Desculpa, não respondo corno"); // if the message is not in a guild (aka in dms), return aka ignore the inputs
   ...
   * */
  const fs = require('fs');
  const { sh_dir, ch_dir, findKey, checkChannel, capitalize, sleep } = require('../../files/utils.js');
  ...
  // Shinyhunt call 
  fs.readFile(sh_dir, function readFileCallback(err, data) {
  ...
  if(command) {
      if(command.whitelistOnly) {
        ...
        if(!whitelistCheck(message.author.id)) return message.channel.send("Comando Restrito!");
      }
